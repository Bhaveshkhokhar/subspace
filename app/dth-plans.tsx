...
import { ArrowLeft, Tv, Clock, Zap, Star, CircleCheck as CheckCircle, CircleAlert as AlertCircle } from 'lucide-react-native';
...
export default function DTHPlansPage() {
  ...
  const operatorCode = params.operatorCode as string;
  ...
  const fetchPlans = async () => {
    ...
    try {
      ...
      const types = fetchPlans?.length > 0 ? Array.from(
        new Set<string>(fetchedPlans?.map((plan: Plan) => plan.type))
      ) : [];
  ...
  const handleRecharge = async () => {
    if (!selectedPlan || !operatorId || !customerNumber) {
  ...
  const getPlanIcon = (plan: Plan) => {
    ...
    return <Tv size={20} color="#10B981" />;
  };
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerContent}>
            <Text style={styles.title}>{operatorName || 'DTH Operator'}</Text>
            {customerNumber && (
              <Text style={styles.customerNumber}>{customerNumber}</Text>
            )}
          </View>
        </View>
      </View>
      ...
      >
        ...
        {plans.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyTitle}>{t('dth.noPlansAvailableTitle') || 'No Plans Available'}</Text>
            <Text style={styles.emptySubtitle}>{t('dth.noPlansAvailable') || 'There are no plans available for this operator at the moment.'}</Text>
          </View>
        ) : filteredPlans.length === 0 ? (
          <View style={styles.emptyState}>
            <Tv size={48} color="#6B7280" />
            <Text style={styles.emptyTitle}>{t('dth.noPlansAvailableTitle') || 'No Plans Available'}</Text>
            <Text style={styles.emptySubtitle}>{t('dth.noPlanForCategory') || 'No plans available for category'} {formatTabName(activeTab)}</Text>
          </View>
        ) : (
              ...
              >
                ...
                <View style={styles.planContent}>
                  ...
                  <View style={styles.planFeatures}>
                    ...
                    {plan.dataBenefit && plan.dataBenefit !== '0' && plan.dataBenefit !== 'NA' && (
                      <View style={styles.featureItem}>
                        <Zap size={14} color="#9CA3AF" />
                        <Text style={styles.featureText}>
                          {t('dth.data') || 'Data'}: {plan.dataBenefit}
                        </Text>
                      </View>
                    )}
                  </View>
